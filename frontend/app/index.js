...
var placeBlockSound = document.createElement("AUDIO");
var perfectPlaceSound = document.createElement("AUDIO");
var gameOverSound = document.createElement("AUDIO");

document.getElementById("game-pause").style.display = "none";
document.getElementById("game-over").style.display = "none";

gameMusic.src = Koji.config.sounds.backgroundMusic;
gameMusic.autoplay = false;

placeBlockSound.src = Koji.config.sounds.placeBlockSound;
placeBlockSound.autoplay = false;

perfectPlaceSound.src = Koji.config.sounds.perfectPlaceSound;
perfectPlaceSound.autoplay = false;

gameOverSound.src = Koji.config.sounds.gameOverSound;
...
var Stage = /** @class */function () {
  function Stage() {
    ...
    this.scene.background = new THREE.TextureLoader().load(Koji.config.images.backgroundFullImage);
    // camera
...
var Game = /** @class */function () {
  ...
  Game.prototype.addBlock = function () {
    ...
    if (lastBlock && lastBlock.state == lastBlock.STATES.MISSED) {
      ...
      pauseButton.innerHTML = '<i class="fas fa-info"></i>';
      return this.endGame();
    }
...
document.getElementById('continue-button').addEventListener('click', function(event){
  ...
  controlPanel.style.display = 'flex';
  game.blocks[game.blocks.length-1].state = 'active';
  pauseButton.innerHTML = '<i class="fas fa-pause"></i>';
  document.getElementById("game-pause").style.display = "none";
});
...
pauseButton.addEventListener('click', function(event){
  event.stopPropagation();
  game.updateState('loading');
  game.blocks[game.blocks.length-1].state = 'stopped';
  pauseButton.innerHTML = '<i class="fas fa-info"></i>';
